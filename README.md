...
RescueClone is a clean-room Windows backup/recovery suite foundation. The current verified slice supports directory image creation, verification, restore, JSON-defined directory backup job runs, read-only restore planning, and durable local operation execution through the GUI, CLI, and PowerShell module.
