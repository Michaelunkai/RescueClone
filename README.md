...
RescueClone is a clean-room Windows backup/recovery suite foundation. The current verified slice supports directory image creation with selectable v1/v2 containers, verification, restore, JSON-defined directory backup job runs, read-only restore planning, durable local operation execution, and read-only volume inventory through the GUI, CLI, and PowerShell module.
